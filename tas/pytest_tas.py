...
def test_tas(dut: IdfDut, log_minimum_free_heap_size: Callable[..., None]) -> None:
    dut.expect('Tas!')
...
def test_tas_linux(dut: IdfDut) -> None:
    dut.expect('Tas!')
...
@pytest.mark.macos
@idf_parametrize('target', ['linux'], indirect=['target'])
def test_tas_macos(dut: IdfDut) -> None:
    dut.expect('Tas!')
...
def test_tas_host(app: QemuApp, dut: QemuDut) -> None:
